...
public class FrmMain_old extends JFrame implements ActionListener {
    ...
    private void reloadPlanTable(){//这是测试数据，需要用实际数替换
        try {
            allPlan= DistributionUtil.planManager.loadAll();
    ...
    private void reloadPlanStepTabel(int planIdx){
        ...
        try {
            planSteps= DistributionUtil.stepManager.loadSteps(curPlan);
        } catch (BaseException e) {
            JOptionPane.showMessageDialog(null, e.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    public FrmMain_old(){
        ...
        this.dataTablePlan.addMouseListener(new MouseAdapter (){
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                FrmMain_old.this.reloadPlanStepTabel(i);
            }
        ...
        JLabel label=new JLabel("尊敬的 " + BeanUser.currentLoginUser.getUser_id()+", 您好! ");//修改成   您好！+登陆用户名
        statusBar.add(label);
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==this.menuItem_AddPlan){
            ...
            try {
                DistributionUtil.planManager.deletePlan(this.curPlan);
            ...
            int i= FrmMain_old.this.dataTableStep.getSelectedRow();
            if(i<0) {
                JOptionPane.showMessageDialog(null, "请选择步骤", "错误",JOptionPane.ERROR_MESSAGE);
                return;
            }
            try {
                DistributionUtil.stepManager.deleteStep(this.planSteps.get(i));
            } catch (BaseException e1) {
                JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            try {
                DistributionUtil.stepManager.startStep(this.planSteps.get(i));
            } catch (BaseException e1) {
                JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            try {
                DistributionUtil.stepManager.finishStep(this.planSteps.get(i));
            } catch (BaseException e1) {
                JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            try {
                DistributionUtil.stepManager.moveUp(this.planSteps.get(i));
            } catch (BaseException e1) {
                JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            try {
                DistributionUtil.stepManager.moveDown(this.planSteps.get(i));
            } catch (BaseException e1) {
                JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
                return;
            }
